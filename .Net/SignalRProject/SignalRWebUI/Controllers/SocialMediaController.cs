...
namespace SignalRWebUI.Controllers
{
    public class SocialMediaController : Controller
    {
        ...
        public async Task<IActionResult> CreateSocialMedia(CreateSocialMediaDto dto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:44335/api/SocialMedia", st);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"https://localhost:44335/api/SocialMedia/{id}");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.GetAsync($"https://localhost:44335/api/SocialMedia/{id}");
            if (response.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateSocialMedia(UpdateSocialMediaDto dto)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(dto);
            StringContent st = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:44335/api/SocialMedia", st);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
